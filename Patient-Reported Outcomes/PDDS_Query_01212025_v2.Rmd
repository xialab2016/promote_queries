...
```
#above chunk of code is adjusted to get first 30 pdds scores
...
test <- test %>% filter(Freq>0)
  #2731 participants with scores (7/27/2024)
...
```{r}
#only run if you need to export
pdds_all
...
```{r}
#OPTIONAL: Combining PDDS file with EHR Linkage ID file

linkagefile <- read.csv("ms_promote_linked_20240522.csv")
linkagefile
linkagefile <- linkagefile %>%
  select(PROMOTE_ID, PATIENT_STUDY_ID) %>%
  rename(id_participant = PROMOTE_ID)
linkagefile

prt_linkage <- merge(pdds_all, linkagefile, by = "id_participant", all.x = TRUE)
prt_linkage

#write.csv(prt_linkage, "all_pdds_linkage_20241024.csv")
```
...
ggplot(most_recent3, aes(score)) +
    ...
    scale_y_continuous(
      ...
      limits = c(0, 1000),
      minor_breaks = seq(0, 1000, by=125)  
      ) +
    labs(title = "Most Recent PDDS Scores") + theme(plot.title = element_text(hjust = 0.5)) + xlab("PDDS Score") + ylab("Number of Participants")

#Export list of recent pdds scores
#ggsave("/Users/jeanninejacokes/Desktop/MS Clinic/pdds_score.png", plot = pdds_score, width = 6.67, height = 4.5, units = "in")
